(()=> {
  ...
  function card(r){
    ...
    return`<article class="card" data-request="${x(r.id)}">
      ...
      <div class="vt-build-origin">
        ...
        <b>${linked}/${items.length} component${items.length===1?"":"s"} retain catalogue identity</b>
      </div>
  ...
  async function convert(id){
    ...
    const ok=await VoltTechDialog.confirm({
      ...
      message:`Customer: ${p.full_name||p.billing_email||"Customer"} · Build: ${r.name||"PC Build"}. The resulting quote keeps a structured link back to this saved build and its catalogue products.`,
    ...
    const items=(r.build_data?.items||[]).map(i=>({
      ...
      source_metadata:{
        builder_category:i.type||null,
        builder_product_id:i.product_id||null
      }
    }));
    ...
    if(quoteId){
      const linked=await c.rpc("admin_link_quote_source",{
        p_quote_id:quoteId,
        p_source_type:"builder",
        p_source_id:r.id,
        p_metadata:{
          saved_build_name:r.name||"PC Build",
          estimated_total:Number(r.estimated_total||0),
          estimated_power_watts:r.estimated_power_watts||null,
          compatibility_status:r.compatibility_status||null,
          component_count:items.length,
          linked_product_count:items.filter(i=>i.product_id).length
        }
      });

      if(linked.error){
        q("#status").textContent=`Quote created, but source provenance could not be linked: ${linked.error.message}`;
        return;
      }
    }
