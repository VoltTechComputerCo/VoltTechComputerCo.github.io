...
async function helix(path: string, key: string, values: string[], bearer: string, clientId: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error("Twitch Helix " + path + " failed: " + res.status + " " + body.slice(0, 300));
  }
  const body = await res.json();
...
Deno.serve(
  withSupabase({ auth: "secret:automations" }, async (req, ctx) => {
    if (req.method !== "POST") {
    ...
    try {
      ...
      for (const row of (previousRows || []) as StatusRow[]) {
        previous.set(row.streamer_login.toLowerCase(), row);
      }

      const bearer = await twitchToken(clientId, clientSecret);
      const streams: any[] = [];
      const users: any[] = [];

      for (const batch of chunks(logins, 100)) {
        streams.push(...await helix("streams", "user_login", batch, bearer, clientId));
        users.push(...await helix("users", "login", batch, bearer, clientId));
      }

      if (users.length < Math.floor(logins.length * 0.8)) {
        throw new Error(
          "Twitch returned only " + users.length + " user records for " +
          logins.length + " configured accounts; refresh aborted"
        );
      }
      ...
      try {
        await ctx.supabaseAdmin.rpc("record_streamer_refresh_result", {
          p_success: false,
          p_error: message
        });
      } catch (_) {}
