...
assert.ok(t.statusDestination({request:{request_number:'REF'},access_token:token}));
assert.equal(t.statusDestination({status_url:'https://evil.test/order-status.html?'+query}), '');
...
console.log('PASS: cart limits/demo/stock, private access, redirect allowlist, production-only delivery rates, strict payment gate and server-derived status');
// Verify preserved core classifies ambiguous submission failures for duplicate prevention.
let status=400;
