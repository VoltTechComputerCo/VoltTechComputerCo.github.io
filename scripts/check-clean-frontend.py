...
def check_reference(source, ref, ids=None):
    ...
    if url.scheme or url.netloc:
        if url.scheme not in ('https', 'mailto'): errors.append(f'{source}: unsupported URL scheme {ref}')
        return
    ...
    if not target.is_file(): errors.append(f'{source.relative_to(ROOT)}: missing {ref}')
    if not url.path and url.fragment and ids is not None and unquote(url.fragment) not in ids: errors.append(f'Missing anchor: {ref}')
    if url.path and url.fragment and target.is_file() and target.suffix == '.html':
        ...
        if unquote(url.fragment) not in target_ids: errors.append(f'Missing destination anchor: {ref}')
...
for source in (ROOT / 'src/pages').glob('*.json'):
    ...
    for tag, attrs in parser.tags:
        ...
        if tag == 'img' and not {'alt', 'width', 'height'} <= attrs.keys(): errors.append('Image needs alt and dimensions')
        if 'style' in attrs or any(key.startswith('on') for key in attrs): errors.append('Inline styling/handler in generated page')
        for key in ('aria-controls', 'aria-labelledby', 'aria-describedby'):
            for ref in attrs.get(key, '').split():
                if ref not in ids: errors.append(f'Missing ARIA target: {ref}')
        if tag == 'label' and attrs.get('for') not in ids: errors.append(f'Missing label target: {attrs}')
    for tag, count in [('h1', 1), ('main', 1), ('header', 1), ('footer', 1)]:
        if sum(t == tag for t, _ in parser.tags) != count: errors.append(f'Expected one {tag}')
    if 'data-vt-shell="clean"' not in text: errors.append('Missing service-worker isolation marker')
    if output.name == 'design-system.html' and 'content="noindex, nofollow"' not in text: errors.append('Inspection page must stay noindex')
    ...
    for tag, attrs in parser.tags:
        ...
        if dep and not dep.startswith('assets/') and dep not in preserved: errors.append(f'Legacy runtime dependency on converted page: {ref}')

for css in (ROOT / 'assets/css').rglob('*.css'):
    for ref in re.findall(r'url\([\'\"]?([^\)\'\"]+)', css.read_text()): check_reference(css, ref)
for js in (ROOT / 'assets/js').rglob('*.js'):
    code = js.read_text()
    for ref in re.findall(r'from\s+[\'\"]([^\'\"]+)', code): check_reference(js, ref)
    pure = js.parent.name != 'services' and js.name != 'home.js'
    if pure and re.search(r'\b(fetch|localStorage|sessionStorage|supabase|gtag)\b', code): errors.append(f'Unexpected integration in pure inspection shell: {js}')
