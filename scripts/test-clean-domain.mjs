...
if(!builder.includes('LEGACY_BASE = "https://volttechcomputerco.github.io/"'))fail('feed legacy normalisation missing');
if(!builder.includes('BASE = "https://volttechcomputerco.co.za/"'))fail('feed canonical base mismatch');
const validator=fs.readFileSync(new URL('../scripts/validate-static.py',import.meta.url),'utf8');
if(!validator.includes('historical github.io canonical remains until Step 8.3 migration'))fail('historical canonical migration allowance missing');
...
console.log('PASS: Step 8.1A canonical source and SEO infrastructure contracts');
