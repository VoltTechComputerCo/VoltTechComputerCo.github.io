import {CATEGORY_ORDER,CATEGORY_LABELS,createEmptyBuild,selectProduct,removeProduct,clearBuild,getProductsByCategory,searchProducts,getBestOffer,getProductPrice,calculateBuildTotal,formatMoney,getSelectedCount,getNextCategory,getCategorySummary,getStockLabel} from "./build-engine.js";
import {getCompatibility,validateBuild,estimatePower} from "./compatibility-engine.js?v=0.5";
import {loadCatalogue} from "./data-loader.js";

const e={steps:document.getElementById("steps"),search:document.getElementById("search"),compatibleOnly:document.getElementById("compatible-only"),categoryTitle:document.getElementById("category-title"),productCount:document.getElementById("product-count"),products:document.getElementById("products"),buildList:document.getElementById("build-list"),total:document.getElementById("total"),power:document.getElementById("power"),report:document.getElementById("report"),clearBuild:document.getElementById("clear-build"),catalogueNote:document.getElementById("catalogue-note")};
...
async function init(){
    ...
    try{
        ...
        currency=d.currency;
        e.catalogueNote.textContent=`${catalogue.length} prototype products loaded across ${d.categoryCount} category files and ${d.supplierCount} supplier feeds · v0.5 confidence-aware compatibility · test pricing and stock only · not a customer quotation.`;
        render();
    }catch(x){
        console.error(x);
        e.catalogueNote.textContent="Prototype catalogue could not be loaded.";
        e.products.innerHTML='<div class="empty">Could not load v0.5 catalogue data.</div>';
    }
}
...
function render(){renderSteps();renderProducts();renderBuild();renderPower();renderReport()}

function renderSteps(){
    e.steps.innerHTML=CATEGORY_ORDER.map((c,i)=>{
        const p=build[c],cl=["step",c===activeCategory?"active":"",p?"complete":""].filter(Boolean).join(" ");
...
function renderProducts(){
    ...
    let ps=searchProducts(getProductsByCategory(catalogue,activeCategory),e.search.value);
    let ev=ps.map(product=>({product,result:getCompatibility(product,build)}));
    let v=e.compatibleOnly.checked?ev.filter(x=>x.result.compatible||build[activeCategory]?.id===x.product.id):ev;
    e.productCount.textContent=`${v.length} of ${ps.length}`;
    if(!v.length){
    ...
    e.products.innerHTML=v.map(({product,result})=>card(product,result)).join("");
    e.products.querySelectorAll("[data-select-product]").forEach(b=>b.addEventListener("click",()=>{
        const product=catalogue.find(x=>x.id===b.dataset.selectProduct);
        if(!product)return;
        ...
        if(!done)activeCategory=getNextCategory(product.type,build);
        e.search.value="";
        render();
        if(done)scrollDone();
    }));
...
function openCategory(c){
    ...
    renderSteps();
    renderProducts();
    requestAnimationFrame(()=>document.querySelector(".catalogue")?.scrollIntoView({behavior:"smooth",block:"start"}));
}

function scrollDone(){
    requestAnimationFrame(()=>requestAnimationFrame(()=>setTimeout(()=>e.total?.scrollIntoView({behavior:"smooth",block:"center"}),120)));
}

function textOf(x){return x?.text||String(x)}

function card(p,r){
    const o=getBestOffer(p),sel=build[p.type]?.id===p.id;
    ...
    const supplier=o?`${o.supplierName||o.source} · ${getStockLabel(o.stockStatus)}`:"No supplier offer";
...
function renderBuild(){
    e.buildList.innerHTML=CATEGORY_ORDER.map(c=>{
        const p=build[c];
        return p?`<div class="build-item"><div><span>${esc(CATEGORY_LABELS[c])}</span><div class="build-price">${esc(formatMoney(getProductPrice(p),currency))}</div></div><b>${esc(p.name)}</b></div>`:`<div class="build-item"><span>${esc(CATEGORY_LABELS[c])}</span><b>Not selected</b></div>`;
    }).join("");
...
function renderReport(){
    const r=validateBuild(build),n=getSelectedCount(build),a=[];
    if(!n){
    ...
    e.report.querySelectorAll("[data-fix-category]").forEach(btn=>{
        btn.addEventListener("click",()=>openCategory(btn.dataset.fixCategory));
    });
}
...
function metaOf(p){
    const s=p.specs||{};
    switch(p.type){
        case"cpu":return[s.socket,s.cores?`${s.cores} cores`:null,s.threads?`${s.threads} threads`:null,s.tdpWatts?`${s.tdpWatts}W TDP`:null].filter(Boolean);
        case"motherboard":return[s.socket,s.chipset,s.formFactor,s.memoryType,s.wifi?"Wi-Fi":null].filter(Boolean);
        case"memory":return[s.capacityGB?`${s.capacityGB}GB`:null,s.modules?`${s.modules} modules`:null,s.memoryType,s.speedMTs?`${s.speedMTs} MT/s`:null].filter(Boolean);
        case"gpu":return[s.vramGB?`${s.vramGB}GB VRAM`:null,s.lengthMm?`${s.lengthMm}mm`:null,s.slots?`${s.slots}-slot`:null,s.recommendedPsuWatts?`${s.recommendedPsuWatts}W PSU`:null].filter(Boolean);
        case"storage":return[s.capacityGB?(s.capacityGB>=1000?`${s.capacityGB/1000}TB`:`${s.capacityGB}GB`):null,s.interface,s.formFactor,s.pcieGeneration?`PCIe ${s.pcieGeneration}.0`:null].filter(Boolean);
        case"psu":return[s.wattage?`${s.wattage}W`:null,s.efficiency,s.modular,s.atxVersion].filter(Boolean);
        case"case":return[s.supportedMotherboardSizes?.join(" / "),s.maxGpuLengthMm?`${s.maxGpuLengthMm}mm GPU`:null,s.maxCpuCoolerHeightMm?`${s.maxCpuCoolerHeightMm}mm cooler`:null].filter(Boolean);
        case"cooler":return[s.coolerType==="aio"?"Liquid AIO":"Air cooler",s.radiatorSizeMm?`${s.radiatorSizeMm}mm radiator`:null,s.heightMm?`${s.heightMm}mm height`:null].filter(Boolean);
        default:return[];
    }
}
