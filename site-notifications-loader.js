(()=>{
  ...
  function addScript(src,test){
    return new Promise((resolve,reject)=>{
      ...
      if(existing){if(test?.()){resolve();return}existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',reject,{once:true});return}
  ...
  // Store discovery is public and intentionally loads before the account check.
  addScript('commerce/js/site-store-entry.js?v=1.0.0',()=>!!window.__vtMoneyMakerEntry).catch(()=>{});

  let hasAuth=false;
  try{hasAuth=Object.keys(localStorage).some(k=>/^sb-.*-auth-token$/.test(k));}catch{}
  if(!hasAuth)return;
  ...
  async function boot(){
    ...
    try{
      if(!window.supabase){await new Promise((resolve,reject)=>{const el=document.createElement('script');el.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';el.onload=resolve;el.onerror=reject;document.head.append(el)})}
      ...
      if(!window.__voltTechOnboardingV1)await addScript('onboarding.js?v=1.0.0',()=>!!window.__voltTechOnboardingV1);
    }catch(e){console.warn('VoltTech account services unavailable',e)}
  }
  boot();
})();
