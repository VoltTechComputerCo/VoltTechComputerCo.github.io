...
(() => {
  ...
  async function loadAddresses() {
    ...
    if (error) {
      list.innerHTML = '<p class="empty error">Could not load saved locations.</p>';
      return;
    }

    if (!data?.length) {
      list.innerHTML = '<p class="empty">No saved delivery locations yet.</p>';
      return;
    }
    ...
    list.innerHTML = data.map(a => `
      <article class="history-card address-card">
        <div class="history-head">
          <div>
            <span class="mini">${escapeHtml(a.label || "Address")}</span>
            <h3>${escapeHtml(a.recipient_name || "Delivery location")}</h3>
          </div>
          ${a.is_default_shipping ? '<span class="pill good">Default delivery</span>' : ""}
        </div>
        ...
        <div class="small-actions">
          <button class="text-btn" data-edit-address="${a.id}">Edit</button>
          <button class="text-btn danger" data-delete-address="${a.id}">Delete</button>
        </div>
      </article>
    `).join("");
  ...
  async function saveAddress(e) {
    ...
    if (result.error) {
      status(`Could not save location: ${result.error.message}`, "error");
      return;
    }
  ...
  async function deleteAddress(id) {
    ...
    if (error) status(`Could not delete location: ${error.message}`, "error");
      ...
      await loadAddresses();
      status("Delivery location deleted.", "success");
    }
  }
  ...
  async function loadOrders() {
    ...
    if (error) {
      target.innerHTML = '<p class="empty error">Could not load purchase history.</p>';
      return;
    }

    if (!data?.length) {
      target.innerHTML = '<p class="empty">No purchases recorded on this account yet.</p>';
      return;
    }
    ...
    target.innerHTML = data.map(o => `
      <article class="history-card">
        <div class="history-head">
          <div><span class="mini">${escapeHtml(o.order_number)}</span><h3>${date(o.placed_at || o.created_at)}</h3></div>
          <span class="pill">${escapeHtml(o.status)}</span>
        </div>
        <div class="history-lines">
          ${(o.order_items || []).map(i => `<div><span>${i.quantity} × ${escapeHtml(i.product_name)}</span><b>${money(i.line_total)}</b></div>`).join("")}
        </div>
        <div class="history-total"><span>${escapeHtml(o.payment_status)}</span><b>${money(o.total)}</b></div>
      </article>
    `).join("");
  }
  ...
  async function loadJobs() {
    ...
    if (error) {
      target.innerHTML = '<p class="empty error">Could not load service history.</p>';
      return;
    }

    if (!data?.length) {
      target.innerHTML = '<p class="empty">No repair or optimisation jobs recorded on this account yet.</p>';
      return;
    }
    ...
    target.innerHTML = data.map(j => `
      <article class="history-card">
        <div class="history-head">
          <div><span class="mini">${escapeHtml(j.job_number)} · ${escapeHtml(labelService(j.service_type))}</span><h3>${escapeHtml(j.title)}</h3></div>
          <span class="pill">${escapeHtml(j.status)}</span>
        </div>
        ${j.device_name ? `<p class="muted-line">${escapeHtml(j.device_name)}</p>` : ""}
        <div class="job-money">
          <span>Quoted <b>${j.amount_quoted == null ? "—" : money(j.amount_quoted)}</b></span>
          <span>Paid <b>${money(j.amount_paid)}</b></span>
          <span>${escapeHtml(j.payment_status)}</span>
        </div>
        ${(j.service_job_updates || []).length ? `
          <div class="timeline">
            ${[...j.service_job_updates].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).map(u => `
              <div class="timeline-item"><i></i><div><b>${escapeHtml(u.title)}</b><small>${date(u.created_at)}${u.note ? " · "+escapeHtml(u.note) : ""}</small></div></div>
            `).join("")}
          </div>` : ""}
      </article>
    `).join("");
  ...
  function labelService(type) {
    const labels = {
      repair: "Repair",
      stream_optimisation: "Stream optimisation",
  ...
  async function saveProfile(e) {
    ...
    if (error) {
      status(`Could not save profile: ${error.message}`, "error");
      return;
    }
  ...
  async function changeEmail(e) {
    ...
    if (!email || email === currentUser.email) {
      status("Enter a different email address first.", "error");
      return;
    }
  ...
  async function refresh() {
    ...
    const src = currentUser.user_metadata?.avatar_url || currentUser.user_metadata?.picture;
    if (src) {
      avatar.src = src;
      avatar.hidden = false;
    }
    ...
    try {
      await Promise.all([loadProfile(currentUser), loadAddresses(), loadOrders(), loadJobs()]);
    } catch (error) {
      console.error(error);
